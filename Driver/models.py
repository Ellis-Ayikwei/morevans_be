...
class Driver(Basemodel):
    ...
    national_insurance_number = models.CharField(
        ...
        null=True, 
        blank=True,
        validators=[RegexValidator(r'^[A-CEGHJ-PR-TW-Z]{1}[A-CEGHJ-NPR-TW-Z]{1}[0-9]{6}[A-D]{1}$', 'Valid NI number required')],
    ...
    date_started = models.DateField(default=None, help_text=_("Date driver started with the company"))
    ...
    license_categories = models.JSONField(
        default=list,
        help_text=_("Categories on driver's license")
    )
    license_expiry_date = models.DateField(default=None, help_text=_("License expiry date"))
    digital_tachograph_card_number = models.CharField(
        max_length=20,
        blank=True,
        help_text=_("Digital tachograph card number")
    )
    tacho_card_expiry_date = models.DateField(
        null=True,
        blank=True,
        help_text=_("Tachograph card expiry date")
    )
    
    # Driver qualifications
    has_cpc = models.BooleanField(
        default=False,
        help_text=_("Driver has Certificate of Professional Competence")
    )
    cpc_expiry_date = models.DateField(
        null=True,
        blank=True,
        help_text=_("CPC qualification expiry date")
    )
    has_adr = models.BooleanField(
        default=False,
        help_text=_("Qualified for dangerous goods transport (ADR)")
    )
    adr_expiry_date = models.DateField(
        null=True,
        blank=True,
        help_text=_("ADR certification expiry date")
    )
    ...
    max_weekly_hours = models.PositiveIntegerField(
        default=48,
        help_text=_("Maximum weekly working hours")
    )
    opted_out_of_working_time_directive = models.BooleanField(
        default=False,
        help_text=_("Driver has opted out of 48-hour working week limit")
    )
    ...
    preferred_vehicle_types = models.JSONField(
        null=True,
        blank=True,
        help_text=_("Preferred vehicle types for this driver")
    )
